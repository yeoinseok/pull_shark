...
`Python` `TensorFlow` `Keras` `MobileNetV2` `OpenCV` `MediaPipe` `ONNX` `TensorRT` `Jetson Nano`
...
<a href="https://youtu.be/wPea2OLFxoQ">
  <img src="./assets/demo_ui.png" width="720" alt="YouTube Demo">
</a>
...
카메라로 입력된 손 이미지를 MediaPipe로 검출하고, MobileNetV2 기반 분류 모델을 통해 한글 지문자 31개 클래스를 예측합니다. 학습된 Keras 모델은 ONNX로 변환한 뒤 Jetson Nano에서 TensorRT 엔진으로 최적화하여 실시간 추론에 사용했습니다.

## 주요 기능

- 실시간 카메라 입력 기반 손 영역 검출
...
<img src="./assets/model_architecture.png" width="720" alt="Model Architecture">

### ONNX 및 TensorRT 변환 흐름

<img src="./assets/onnx_flow.png" width="720" alt="ONNX and TensorRT Flow">

### 입력 및 전처리

<img src="./assets/preprocessing.png" width="720" alt="Preprocessing">

### UI 구성

<img src="./assets/ui_design.png" width="720" alt="UI Design">
...
초기에는 클래스별 50장에서 100장 사이의 데이터를 수집했으나, 추론 과정에서 일부 클래스의 인식률이 낮아 클래스별 데이터를 150장에서 200장 이상으로 확장했습니다. 이후 일부 데이터는 RGB 변환과 위치 변화 등을 적용하여 약 400장 수준으로 증강했습니다.
